...
public:
	...
	bool SetData( unsigned int nValue ) { return x_SetData(m_iPos,nValue); };

	//JXH
#ifdef WIN32
	bool SetData( unsigned __int64 nValue ) { return x_SetData(m_iPos,nValue); };
#else
	bool SetData( uint64_t nValue ) { return x_SetData(m_iPos,nValue); };
#endif

	bool SetChildData( unsigned int nValue ) { return x_SetData(m_iPosChild,nValue); };
...
#define MARKUP_SETDEBUGSTATE
	...
	//JXH
	bool x_AddElem_UnsignedInt( MCD_PCSZ pName, unsigned int nValue, int nFlags );
	...
	bool x_SetData( int iPos, unsigned int nValue );
#ifdef WIN32
	bool x_SetData( int iPos, unsigned __int64 nValue );
#else
	bool x_SetData( int iPos, uint64_t nValue );
#endif
...
public:
	int GetValueByTag(string strTag, string& strValue);
	int SetValueByTag(string strTag, string strValue);
	int InsertNewTag(string strTag, string strValue);
	int ExtractSubString(string& strItem, string strTag, int index, char sep);

	int SetValueByTag(string strTag, unsigned int iValue);
	int InsertNewTag(string strTag, unsigned int iValue);

	//JXH 64bit
	bool InsertElem( MCD_CSTR szName, unsigned int nValue, int nFlags=0 ) { return x_AddElem_UnsignedInt(szName,nValue,nFlags|MNF_INSERT); };

#ifdef WIN32
	bool CMarkup::x_AddElem_UnsignedInt64( MCD_PCSZ pName, unsigned __int64 nValue, int nFlags )
#else
	bool x_AddElem_UnsignedInt64( MCD_PCSZ pName, uint64_t nValue, int nFlags )
#endif
	{
		// Convert integer to string
		MCD_CHAR szVal[64];
#ifdef WIN32
		MCD_SPRINTF( MCD_SSZ(szVal), MCD_T("%I64u"), nValue );
#else
		MCD_SPRINTF( MCD_SSZ(szVal), MCD_T("%lu"), nValue );
#endif
		return x_AddElem( pName, szVal, nFlags );
	}

#ifdef WIN32
	bool InsertElem( MCD_CSTR szName, unsigned __int64 nValue, int nFlags=0 ) { return x_AddElem_UnsignedInt64(szName,nValue,nFlags|MNF_INSERT); };
	bool AddElem( MCD_CSTR szName, unsigned __int64 nValue, int nFlags=0 ) { return x_AddElem_UnsignedInt64(szName,nValue,nFlags); };
	bool AddElem( MCD_CSTR szName, unsigned int nValue, int nFlags=0 ) { return x_AddElem_UnsignedInt(szName,nValue,nFlags); };
	
	int InsertNewTag(string strTag, unsigned __int64 iValue);
	int SetValueByTag(string strTag, unsigned __int64 iValue);
#else
	bool InsertElem( MCD_CSTR szName, uint64_t nValue, int nFlags=0 ) { return x_AddElem_UnsignedInt64(szName,nValue,nFlags|MNF_INSERT); };
	bool AddElem( MCD_CSTR szName, uint64_t nValue, int nFlags=0 ) { return x_AddElem_UnsignedInt64(szName,nValue,nFlags); };
	bool AddElem( MCD_CSTR szName, unsigned int nValue, int nFlags=0 ) { return x_AddElem_UnsignedInt(szName,nValue,nFlags); };
	
	int InsertNewTag(string strTag, uint64_t iValue);
	int SetValueByTag(string strTag, uint64_t iValue);
#endif
};
