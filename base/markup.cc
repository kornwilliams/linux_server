...
bool CMarkup::x_SetData( int iPos, unsigned int nValue )
{
	// Convert integer to string
	MCD_CHAR szVal[25];
	MCD_SPRINTF( MCD_SSZ(szVal), MCD_T("%u"), nValue );
	return x_SetData( iPos, szVal, 0 );
}

#ifdef WIN32
bool CMarkup::x_SetData( int iPos, unsigned __int64 nValue )
#else
bool CMarkup::x_SetData( int iPos, uint64_t nValue )
#endif
{
	// Convert integer to string
	MCD_CHAR szVal[64];
...
bool CMarkup::x_AddElem_UnsignedInt( MCD_PCSZ pName, unsigned int nValue, int nFlags )
{
	// Convert integer to string
	MCD_CHAR szVal[25];
	MCD_SPRINTF( MCD_SSZ(szVal), MCD_T("%u"), nValue );
...
//??strTag?в??ҵ?index??item??????strItem?У???sep??Ϊ?ָ???
int CMarkup::ExtractSubString(string& strItem, string strTag, int index, char sep)
{
	int i = 0;
	int iPos = 0, iNext = 0;
	for(i = 0; i < index; i++)
	...
	if( iNext < 0)
	{
		...
		strItem = strTag.substr(iPos);
	}
	else
		strItem = strTag.substr(iPos, iNext - iPos);

	return 0;
}

int CMarkup::GetValueByTag(string strTag, string& strValue)
{
	strValue="";
	ResetPos();
	...
	while(1)
	{
		ExtractSubString(strItem, strTag, i, '/');//?ӵ?һ???ӽڵ㿪ʼ????
		i++;
		if(strItem=="")//??????iλ??û?У?˵???Ѿ??????ײ??ڵ?
			break;
		if(FindChildElem(strItem.data()))
			IntoElem();//??????һ??
		else
			return 0;//δ?ҵ???????
	}
...
int CMarkup::SetValueByTag(string strTag, string strValue)
{
	ResetPos();
	...
	SetData(strValue.data());
	return 0;
}

int CMarkup::SetValueByTag(string strTag, unsigned int iValue)
{
	ResetPos();
...
#ifdef WIN32
int CMarkup::SetValueByTag(string strTag, unsigned __int64 iValue)
#else
int CMarkup::SetValueByTag(string strTag, uint64_t iValue)
#endif
{
	ResetPos();
...
int CMarkup::InsertNewTag(string strTag, string strValue)
{
	ResetPos();
	string strItem;
	int i=1;
	...
	while(1)
	{
		ExtractSubString(strItem, strTag, i, '/');
		i++;
		if(strItem=="")
			break;
		if(!FindElem(strItem.data()))
			AddElem(strItem.data());
		IntoElem();
	}

	//????Value
	//SetData(strValue);
	SetValueByTag(strTag,strValue);
	return 0;
}

#ifdef WIN32
int CMarkup::InsertNewTag(string strTag, unsigned __int64 iValue)
#else
int CMarkup::InsertNewTag(string strTag, uint64_t iValue)
#endif
{
	ResetPos();
	string strItem;
	...
	//????Value
	//SetData(strValue);
	SetValueByTag(strTag, iValue);
	return 0;
}

int CMarkup::InsertNewTag(string strTag, unsigned int iValue)
{
	ResetPos();
	string strItem;
	int i=1;
